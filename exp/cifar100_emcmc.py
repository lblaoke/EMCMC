...
parser.add_argument('--save'            , type=str  , default='cifar100_emcmc'  )
...
for epoch in range(args.epoch):
    ...
    if (epoch%50)+1>48:
        ...
        w_list.append(utils.save_sample(net_s, f'{path}/s{epoch}.pt'))

        # anchor
        acc_a = test_eval.test(args.gpu,net_a,testloader,oodloader)
        w_list.append(utils.save_sample(net_a, f'{path}/a{epoch}.pt'))
...
# final testing
print('\n==> Final Testing...')
test_eval.multi_test(args.gpu, net_s, w_list, testloader, oodloader)
