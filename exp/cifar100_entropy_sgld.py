...
net_sampler = models.ResNet18(num_classes=100).to(args.gpu)
net_mean = models.ResNet18(num_classes=100).to(args.gpu)
...
def test(net,OOD:bool=False):
    ...
    acc = metric.evaluate(output,correct,val_targets,uncertainty)
...
pred = torch.argmax(fake,dim=1)
correct = (pred==val_targets)
uncertainty = -torch.sum(F.softmax(fake,dim=1)*F.log_softmax(fake,dim=1),dim=1)

print('\nTest Set:')
metric.evaluate(fake,correct,val_targets,uncertainty)

# OOD set
fake_ood = sum(pred_list_ood)/len(pred_list_ood)
correct = torch.ones_like(val_targets)
correct_ood = torch.zeros_like(val_targets_ood)
uncertainty_ood = -torch.sum(F.softmax(fake_ood,dim=1)*F.log_softmax(fake_ood,dim=1),dim=1)
